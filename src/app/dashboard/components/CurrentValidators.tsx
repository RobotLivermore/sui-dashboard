...
import { CurrentValidatorQuery, currentValidatorQuerySql } from '../utils/currentValidatorsQuery'

export default function CurrentValidators() {

  const queryTaskVaule = useContext(QueryTaskContext)
  
  const task = queryTaskVaule?.tasks.find(t => t.id === CurrentValidatorQuery.id)
  ...
  const columns = [{ title:"Name", dataIndex: 'name' }, { title: "Stake", dataIndex: 'stake_balance' }];
  return (
    <GridItemContentContainer title="Current Validators" outerUrl={`https://console.chainbase.com/dataCloud?sql=${encodeURIComponent(currentValidatorQuerySql)}`} dragable>
